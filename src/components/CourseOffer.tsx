  ...
  return (
    <section id="curso" className="py-20 px-6 max-w-5xl mx-auto w-full mb-20">
      ...
      <div className="bg-pch-card border border-pch-primary/30 rounded-[40px] p-8 md:p-12 shadow-[0_0_50px_rgba(87,204,153,0.1)] max-w-lg mx-auto relative">
        ...
        <h3 className="text-2xl font-bold text-white mb-2">{t.course.course_name}</h3>
        <p className="text-foreground mb-8">{t.course.course_desc}</p>
        
        <div className="flex items-baseline gap-2 mb-8">
          <span className="text-5xl font-bold text-white">{t.course.price}</span>
          <span className="text-foreground">{t.course.payment_type}</span>
        </div>

        <ul className="space-y-4 mb-10">
          {t.course.features.map((feature, i) => (
        ...
        <p className="text-xs text-foreground text-center mt-4 opacity-70">{t.course.stripe_msg}</p>
      </div>
    </section>
  );
};
